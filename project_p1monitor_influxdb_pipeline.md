...
metadata: 
  ...
  modified: 2026-09-22T11:04:41.336Z
---
...
- Full live `telegraf.conf` (`/share/Container Data/telegraf/telegraf.conf` on the QNAP, container `telegraf`, `[agent] interval/flush_interval = "10s"`) has 33 `[[inputs.mqtt_consumer]]` blocks total: the 28 P1Monitor ones above (all `data_format = "value"`, per-topic `data_type` float or string) plus 5 more added later via MQTT Statestream (`topics = ["homeassistant/sensor/<x>/state"]`, base topic "homeassistant"): `solaredge_i1_ac_power` (own `client_id`, `resolution: "live"`) and 4 EMHASS ones tagged `resolution: "forecast"` with an `emhass_` entity_id prefix - `p_load_forecast`, `p_pv_forecast`, `p_grid_forecast`, `total_cost_fun_value` (all float) and `optim_status` (string, own `client_id`, same string/float-never-mixed reasoning as `tarifcode`).

**Known cosmetic Telegraf error (investigated 2026-09-22, not fully root-caused, left as-is per user):** `[inputs.mqtt_consumer] Error in plugin: strconv.ParseFloat: parsing "unavailable": invalid syntax` recurs roughly every ~3 days around 19:38 local (seen 2026-09-18 and 2026-09-21), alongside separate/unrelated `context deadline exceeded` write timeouts to `outputs.influxdb_v2` recurring around 01:00-01:10 local on the same two dates (cause of that one not investigated - possibly a QNAP nightly job). For the "unavailable" one: only the 5 HA-Statestream-fed float topics (`solaredge_i1_ac_power` + the 4 EMHASS forecast sensors) can plausibly emit a literal `"unavailable"` string - none of the 28 raw P1Monitor topics can (the dongle always sends real numbers). Checked in HA history/logbook: `sensor.solaredge_i1_ac_power` shows no state change or gap in that window on 2026-09-18; the 3 EMHASS forecast sensors (`p_load_forecast`/`p_pv_forecast`/`p_grid_forecast`) all step to new values cleanly at ~19:30 (EMHASS's forecast-republish cycle, ~8 min before the error) with no visible gap either - HA's history chart likely smooths over a sub-minute unavailable flicker, so this couldn't be nailed down further without enabling Telegraf's `debug = true` and catching it live next time it recurs.

**Data-loss check for the `context deadline exceeded` timeouts (verified 2026-09-22):** queried InfluxDB directly (`measurement == "W" and entity_id == "p1monitor_consumption_kw"`, `elapsed(unit: 1s)` then `filter(fn: (r) => r.elapsed > 60)`) across 00:30-02:00 local on both 2026-09-18 and 2026-09-21 (the nights the timeouts occurred) - **zero gaps larger than 60s on either night**. So despite the write-timeout errors in the Telegraf logs, no data was actually lost (Telegraf's retry/buffer recovered every time) - both this and the "unavailable" ParseFloat error are confirmed cosmetic/harmless. User decided not to pursue the root cause (QNAP nightly job?) further - not worth more effort unless data loss is ever actually observed.
...
**P1 Monitor's real schema - two sources confirmed 2026-09-22:**
1. A downloadable *export* (`p1mon-sql-export<ts>.zip`) - a zip of per-table files, each a sequence of `replace into <table> (...) values (...)` (or `update config set ...` for config) statements - not a real `mysqldump` (no `CREATE TABLE`), misidentified as "CSV" by the `file` command despite being SQL. Its `e_history_min` (minute resolution) goes all the way back to **2020-01-01**.
2. The **live SQLite databases themselves** (e.g. `e_historie.db`, confirmed via `file`/Python's `sqlite3` module - NOT MySQL/MariaDB as first assumed). `e_historie.db` contains 5 tables: `e_history_min` (minute, only ~1 month rolling retention), **`e_history_uur`** (hour, already aggregated, 2023-09-22 to present), `e_history_dag` (day, already aggregated, **2020-10-09** to present), plus `e_history_maand`/`e_history_jaar`. This is the better source for a backfill - no manual aggregation needed.

Both `e_history_min`/`e_history_uur`/`e_history_dag` share columns `TIMESTAMP, VERBR_KWH_181, VERBR_KWH_182, GELVR_KWH_281, GELVR_KWH_282, VERBR_KWH_X, GELVR_KWH_X, TARIEFCODE, VERBR_GAS_2421` (`e_history_min` additionally has `ACT_VERBR_KW_170`/`ACT_GELVR_KW_270`, instantaneous kW). `_181`/`_182`/`_281`/`_282` are cumulative meter readings (DSMR OBIS 1.8.1/1.8.2/2.8.1/2.8.2). **`VERBR_KWH_X`/`GELVR_KWH_X` are already the per-period consumption/delivery delta** (verified by hand at minute level: a row's `GELVR_KWH_X` equals that row's cumulative `GELVR_28x` reading minus the previous row's, for whichever tariff `TARIEFCODE` is active) - `e_history_uur`/`e_history_dag` already have this pre-aggregated per hour/day, so it maps directly onto InfluxDB's `verbr_kwh_periode` field with **no SQL aggregation needed**. Other tables in the export zip: `weer_history_uur`/`weer` (weather), `temperatuur`, `faseminmax_dag` (per-phase daily min/max), `powerproduction_solar`, `watermeter`, `e_financieel_dag` (daily cost), `statistics`, `config`.

The working Telegraf config lives at `/home/wilco/github/P1monitor/p1monitor-historical-import.conf`: `driver = "sqlite"`, `dsn` = plain path to `e_historie.db` (no MySQL restore needed), one query reading `e_history_uur` tagged `resolution: "hour"` and one reading `e_history_dag` (`WHERE TIMESTAMP < '2023-09-22 00:00:00'`, to avoid double-counting the overlap) tagged `resolution: "day"`. **`time_format = "2006-01-02 15:04:05"` is required** on `time_column` - without it, Telegraf's sql input fails with "parsing time failed: invalid number" on the TEXT timestamp column. **Verified 2026-09-22 with `telegraf --test`** (Telegraf 1.33.3 linux/amd64, just the release tarball, no install needed): produces exactly 25992 hour-points + 1069 day-points, no errors, matching the real row counts - only the actual InfluxDB write is unverified (no network path from the Fedora laptop to `172.10.1.23`, token in the config is a placeholder). If more history is ever wanted for EMHASS's ML model, the export zip's `e_history_min` has genuine minute-resolution data back to 2020-01-01 - much further than InfluxDB's current 2023-08-25 backfill start.
