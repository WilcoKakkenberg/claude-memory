...
metadata: 
  ...
  modified: 2026-09-22T10:27:01.651Z
---
...
**Historical-import (from a P1 Monitor MySQL/MariaDB dump):** separate from the live MQTT->Telegraf pipeline above, there used to be a local historical import (on the old Windows 11 laptop, `C:\Users\Wilco\Documents\P1monitor\`) that loaded readings from a P1 Monitor MySQL/MariaDB dump into InfluxDB (measurement `electricity`, field `verbr_kwh_periode`, tagged `resolution: "hour"`/`"day"` - this is the data the 2026-09-07 Flux backfill in [project_emhass_ml_forecast_refit](project_emhass_ml_forecast_refit.md) later converted into the `p1monitor_consumption_kw` schema). **Mechanism confirmed by user 2026-09-22: this went via Telegraf's `[[inputs.sql]]` plugin (consistent with Telegraf already being the tool for MQTT->InfluxDB in this pipeline), NOT a standalone script** - an earlier reconstruction attempt as a Python script was wrong and has been replaced. The real table/column names in the original dump were never recorded. Since moving to a Fedora laptop, a from-scratch Telegraf config template reproducing the known target result lives at `/home/wilco/github/P1monitor/p1monitor-historical-import.conf` - untested, with placeholder table/column names that need adjusting against the real dump schema before use (run as a one-off `telegraf --config ... --once` against a locally restored copy of the dump).
